...
@SuppressLint("InflateParams") public class RouteActivity extends Activity implements 
OnRouteSearchListener,OnPoiSearchListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
	   ...
	   busStationList=new ArrayList<BusStationList>();
	   walkID=new ArrayList<String>();
	   walkStepLists=new ArrayList<WalkStepList>();
	   poiBikes=new ArrayList<PoiBike>();
	   mMapView.onCreate(savedInstanceState); 
	   walkview=LayoutInflater.from(this).inflate(R.layout.walk_listview, null);
	   pathlist=(ListView) walkview.findViewById(R.id.walklistview);
	   busview=LayoutInflater.from(this).inflate(R.layout.bus_listview, null);
	   buspathlist=(ListView) busview.findViewById(R.id.buslistview);
	   mixview=LayoutInflater.from(this).inflate(R.layout.mix_listview, null);
	   ...
	   ar.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				Intent intent=new Intent(RouteActivity.this,ARNaviActivity.class );
	   ...
	   detail.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
		        if(mode.equals("walk")){
		        	dialogBuilder
		        	.withTitle("路线详情")                                  
	                .setCustomView(walkview, RouteActivity.this)
	                ...
	                .show();  
		        	 ...
		        	 dialogBuilder
		                .withTitle("路线详情")                                               
		                .setCustomView(busview, RouteActivity.this)
		                .isCancelableOnTouchOutside(true)                          
		                .withDuration(700)                                         
		        	 ...
		        	 dialogBuilder
		                .withTitle("路线详情")                                                             
		                .setCustomView(mixview, RouteActivity.this)
		                .isCancelableOnTouchOutside(true)                          
		                .withDuration(700)                                         
	   ...
	   mixpathlist.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				for(int i=0;i<busID.size();){
					if((arg2+"").equals(busID.get(i))||((arg2+1)+"").equals(busID.get(i))){
						 //开始搜索公交站点相关数据
						for(int j=0;j<busStationList.size();j++){
							//找到对应的站点，获取详情
							if((busStationList.get(j).getBusLineId()+"").equals(busID.get(i+1))){
								busdetailInstructions.clear();
						        for(int k=0;k<busStationList.get(j).getBusStationItems().size();k++){
						        ...
						        ListView busdetaillist=new ListView(RouteActivity.this);
						    	BusDetailAdapter busdetailadapter=new BusDetailAdapter
						    			(RouteActivity.this, busdetailInstructions);
						        busdetaillist.setAdapter(busdetailadapter);
								NiftyDialogBuilder busBuilder=new NiftyDialogBuilder(RouteActivity
										.this,R.style.dialog_untran);
								busBuilder
								.withTitle(busStationList.get(j).getBusLineName())                                 
						        .setCustomView(busdetaillist, RouteActivity.this)
						        .isCancelableOnTouchOutside(true)                         
						        .withDuration(700)                                      
				...
				for(int i=0;i<walkID.size();){
					if((arg2+"").equals(walkID.get(i))){
						for(int j=0;j<walkStepLists.size();j++){
							//找到对应的站点，获取详情
							if((walkStepLists.get(j).getWalkId()+"").equals(walkID.get(i+1))){
								walkDetailInstructions.clear();
						        for(int k=0;k<walkStepLists.get(j).getWalkstep().size();k++){
						        	WalkDetailInstruction instruction=new WalkDetailInstruction();
						        	if(!walkID.get(i+1).contains("骑行") && !walkID.get(i+1).contains("bus")){
						        		if(k==walkStepLists.get(j).getWalkstep().size()-1){
					                		String last=walkStepLists.get(j).getWalkstep()
					                				.get(k).getInstruction().substring(0, 
					                						walkStepLists.get(j).getWalkstep()
							                				.get(k).getInstruction().length()
							                				-3)+poiBikes.get(0).getBikeRental();
					                		instruction.setName(last);
					                	}
							        	else
							        		instruction.setName(walkStepLists.get(j).
							        				getWalkstep().get(k).getInstruction());
							            walkDetailInstructions.add(instruction);
						        	}
						        	else if(walkID.get(i+1).contains("bus")){
						        		instruction.setName(walkStepLists.get(j).
						        				getWalkstep().get(k).getInstruction());
						                walkDetailInstructions.add(instruction);
						        	}
						        	else{
						        		String[] bicyclepath=new String[walkStepLists.get(j).getWalkstep()
										                				.get(k).getInstruction().length()];
					 	            	for(int l=0;l<walkStepLists.get(j).getWalkstep()
				                				.get(k).getInstruction().length();l++){
					 	            		bicyclepath[l]=walkStepLists.get(j).getWalkstep()
					                				.get(k).getInstruction().substring(l, l+1);
					 	            		//将步行数据修改为骑行数据（伪的）
					                	...
					                	if(k==walkStepLists.get(j).getWalkstep().size()-1){
					                		String last=bicycleinstruction.substring(0, bicycleinstruction
					                				.length()-3)+poiBikes.get(1).getBikeRental();
					                		instruction.setName(last);
					                	}
					                	else
						        ...
						        ListView walkdetaillist=new ListView(RouteActivity.this);
						    	WalkDetailAdapter walkdetailadapter=new WalkDetailAdapter
						    			(RouteActivity.this, walkDetailInstructions);
						        walkdetaillist.setAdapter(walkdetailadapter);
								NiftyDialogBuilder busBuilder=new NiftyDialogBuilder(RouteActivity
										.this,R.style.dialog_untran);
								busBuilder
								.withTitle("详情")                                 
						        .setCustomView(walkdetaillist, RouteActivity.this)
						        .isCancelableOnTouchOutside(true)                         
						        .withDuration(700)                                      
						        .withEffect(Effectstype.Slidetop)                         
						        .show();
	   ...
	   buspathlist.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
				for(int i=0;i<busID.size();){
	...
	public void onWalkRouteSearched(WalkRouteResult walkRouteResult, int rCode) {
    	...
    	if (rCode == 0 && walkRouteResult != null && walkRouteResult.getPaths() != null 
    			&& walkRouteResult.getPaths().size() > 0) {
            WalkPath walkPath = walkRouteResult.getPaths().get(0);
            if(mode.equals("walk")){
            path.setText("共"+df.format(walkPath.getDistance()/1000)+"公里"+"|约"+walkPath.getDuration()/3600+
            		"小时"+walkPath.getDuration()%3600/60+"分钟");
            }
            else if(mode.equals("bicycle")){
                path.setText("共"+df.format(walkPath.getDistance()/1000)+"公里"+"|约"+walkPath.getDuration()/7200+
                		"小时"+(walkPath.getDuration()/2)%3600/60+"分钟");
            }
            //mix模式下的第一次路径规划
                ...
                WalkRouteOverlay walkRouteOverlay = new WalkRouteOverlay(
                        this, mAMap, walkPath, walkRouteResult.getStartPos(),
                ...
                //再接着规划两自行车租赁点之间线路
                startPoint = new LatLonPoint(poiBikes.get(0).getPosition().getLatitude(),
                		poiBikes.get(0).getPosition().getLongitude());
                LatLonPoint endPoint = new LatLonPoint(poiBikes.get(1).getPosition().getLatitude(),
                		poiBikes.get(1).getPosition().getLongitude());
            ...
            //mix模式下的第二次路径规划
                ...
                startPoint = new LatLonPoint(poiBikes.get(1).getPosition().getLatitude(),
                		poiBikes.get(1).getPosition().getLongitude());
                fromAndTo = new FromAndTo(startPoint, endPoint);
                RouteSearch.BusRouteQuery query = new RouteSearch.BusRouteQuery(fromAndTo, busMode, city,0);
                routeSearch.calculateBusRouteAsyn(query);     
            ...
            if(mode.equals("walk")||mode.equals("bicycle")){
	            ...
	            for(int i=0;i<walkPath.getSteps().size();i++){
	            	...
	            	if(mode.equals("bicycle")){
	 	            	String[] bicyclepath=new String[walkPath.getSteps().get(i).getInstruction().length()];
	 	            	for(int j=0;j<walkPath.getSteps().get(i).getInstruction().length();j++){
	 	            		bicyclepath[j]=walkPath.getSteps().get(i).getInstruction().substring(j, j+1);
	 	            		//将步行数据修改为骑行数据（伪的）
	            		...
	            		instruction.setName(walkPath.getSteps().get(i).getInstruction());
	...
	public void onBusRouteSearched(BusRouteResult busRouteResult, int rCode) {
    	...
    	if(rCode == 0 && busRouteResult != null && busRouteResult.getPaths() != null &&
    		...
    		Intent intent =getIntent();
    	    /*取出Intent中附加的数据*/
    	    int pathnum = intent.getIntExtra("buspathnum", 0);
            BusPath busPath = busRouteResult.getPaths().get(pathnum);
            //取出对应方案中所有路段的公交线路名
				...
				if(busPath.getSteps().get(j).getBusLine()!=null){
					...
					for(int k=0;k<busPath.getSteps().get(j).getBusLine()
						...
						if(!busPath.getSteps().get(j).getBusLine().getBusLineName()
							...
							busname+=busPath.getSteps().get(j).getBusLine().getBusLineName()
    	                            .substring(k, k+1);
						else
							break;
					} 	
            ...
            if(mode.equals("bus")){
            	path.setText("共"+df.format(busPath.getDistance()/1000)+"公里"+"|约"+busPath.getDuration()/3600+
            		"小时"+busPath.getDuration()%3600/60+"分钟|步行"+(int)busPath.getWalkDistance()+"米");
            ...
            //mix模式下的第三次路径规划
            	...
            	if(poiBikes.size()>0)
            		path.setText("公共自行车&"+busname+"|"+df.format(mixdistance)+"公里"+"|步行"+
            	        df.format(mixwalkdistance)+"公里|骑行"+df.format(mixbicycledistance
            	        		)+"公里|约"+mixtime/3600+"小时"+mixtime%3600/60+"分钟");
            	else
            		path.setText("共"+df.format(busPath.getDistance()/1000)+"公里"+"|约"+busPath.getDuration()/3600+
                    		"小时"+busPath.getDuration()%3600/60+"分钟|步行"+(int)busPath.getWalkDistance()+"米");
            }
            ...
            for(int i=0;i<busPath.getSteps().size();i++){
        		...
        		if(mode.equals("bus")){
	                ...
	                instruction.setName("步行"+(int)busPath.getSteps().get(i).getWalk()
	                		.getDistance()+"米");
	                instruction.setImgId(R.drawable.walk);
	                busInstructions.add(instruction); 
        			WalkStepList stationList=new WalkStepList();
            		stationList.setWalkId(busPath.getSteps().get(i).getWalk().toString());
            		stationList.setWalkstep(busPath.getSteps().get(i).getWalk().getSteps());
            		//将数据填充进去，方便后续点击时直接获取
            		walkStepLists.add(stationList);  
	                ...
	                mixInstructions.add(instruction);
	            	WalkStepList stationList=new WalkStepList();
            		stationList.setWalkId(busPath.getSteps().get(i).getWalk().toString()+"bus");
            		stationList.setWalkstep(busPath.getSteps().get(i).getWalk().getSteps());
            		//将数据填充进去，方便后续点击时直接获取
            		walkStepLists.add(stationList);  
            		walkID.add(mixInstructions.size()-1+"");
            		walkID.add(busPath.getSteps().get(i).getWalk().toString()+"bus");
        		}	 
            	//公交数据
            	if(busPath.getSteps().get(i).getBusLine()!=null){
            		...
            		stationList.setBusStationItems(busPath.getSteps()
            				.get(i).getBusLine().getPassStations());
            		stationList.setPassStationNum(busPath.getSteps()
            				.get(i).getBusLine().getPassStationNum());
            		//将数据填充进去，方便后续点击时直接获取
            		...
            		if(mode.equals("bus")){
	            		...
	            		//取出对应方案中路段的公交线路名
	            		String buslinename="";
    					for(int k=0;k<busPath.getSteps().get(i).getBusLine()
    							.getBusLineName().length()-1;k++){
    						if(!busPath.getSteps().get(i).getBusLine().getBusLineName()
    	                            .substring(k, k+1).equals("("))
    							buslinename+=busPath.getSteps().get(i).getBusLine().getBusLineName()
        	                            .substring(k, k+1);
    						else
    							break;
    					} 	
		                start.setName(busPath.getSteps().get(i).getBusLine()
		                		.getDepartureBusStation().getBusStationName()+"站上车"+"(乘坐"+
		                		buslinename+")");
		                ...
		                arrive.setName(busPath.getSteps().get(i).getBusLine()
		                		.getArrivalBusStation().getBusStationName()+"站下车");
		                arrive.setImgId(R.drawable.car);
		                busInstructions.add(arrive);
		                busID.add(busInstructions.size()-1+"");
		                busID.add(busPath.getSteps().get(i).getBusLine().getBusLineId());
            		}
            		else if(mode.equals("mix")){
            ...
            if(mode.equals("bus")){
	            ...
	            BusRouteOverlay routeOverlay = new BusRouteOverlay(
	                    this, mAMap, busPath,busRouteResult.getStartPos(),
	...
	public void onPoiSearched(PoiResult result, int rCode) {
		if (rCode == 0 && result != null && result.getQuery() != null && result.getQuery()
				.equals(query)) {// 搜索poi的结果
			poiResult = result;
			// 取得搜索到的poiitems有多少页
			...
			if (isfirstbike && poiItems != null && poiItems.size() > 0) {
			   ...
			   doSearchQuery(new LatLonPoint((startPoint.getLatitude()+endPoint
					   .getLatitude())/2, (startPoint.getLongitude()+endPoint.getLongitude
							   ())/2));
			}
			   ...
			   poiBikes.add(bike);
			   //先规划步行路径
			   LatLonPoint endPoint = new LatLonPoint(poiBikes.get(0).getPosition().getLatitude(),
					   poiBikes.get(0).getPosition().getLongitude());
			   fromAndTo = new FromAndTo(startPoint, endPoint);
			   WalkRouteQuery query = new WalkRouteQuery(fromAndTo,walkMode);
	           routeSearch.calculateWalkRouteAsyn(query);
			} 
			else if((isfirst && poiBikes.size()==0) || (!isfirstbike && poiBikes.size()<2)){
				poiBikes.clear();
				//没有自行车数据的话直接规划公交线路
		        RouteSearch.BusRouteQuery query = new RouteSearch.BusRouteQuery(fromAndTo, busMode,city,0);
		        routeSearch.calculateBusRouteAsyn(query);
			...
			Toast.makeText(RouteActivity.this, "请确认是否联网",Toast.LENGTH_SHORT).show();
		}
	}
